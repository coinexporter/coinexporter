...
<div class="welcome-dashboard">
	<div class="container">
  @section('title', 'Job Done Request')
                ...
                @foreach ($JobPaymentChecks as $key=>$JobPaymentCheck)
                  ...
                  <td>{{$JobPaymentCheck->userid}} </td>
                    ...
                    <td>
                      ...
                      <p>N/A</p>
                      @else
                      <a href="#" class="click"  data-bs-toggle="modal" data-bs-target="#rejectModal{{$key+1}}">click</a>
                      
                      @endif
                    </td>
                    @if($JobPaymentCheck->tvl_status == 'Approved')
                    <td></td>
                    @else
...
<script>
  ...
  $.ajax({
    headers: {
            ...
            $("#successmsg").html('<div class="alert alert-success alert-dismissible" role="alert"><button trype="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button><strong>Status updated Successfully!</strong></div>');  
