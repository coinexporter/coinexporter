	...
	<div class="page-header">
		<div class="card breadcrumb-card">
			<div class="row justify-content-between align-content-between" style="height: 100%;">
				<div class="col-md-6">
					<h3 class="page-title">{{__('activeusers.index.title')}}</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item">
							<a href="{{ route('dashboard') }}">Dashboard</a>
						</li>
						<li class="breadcrumb-item active-breadcrumb">
							<a href="{{ route('activeusers.index') }}">{{ __('activeusers.index.title') }}</a>
						</li>
					</ul>
				</div>
						...
						<tbody>
							@foreach($userpromotors as $userpromotor)
								<tr>
									...
									<span class="badge @if($userpromotor->status == 'active')bg-success @endif">{{ucfirst($userpromotor->status)}}</span>
									</td>
